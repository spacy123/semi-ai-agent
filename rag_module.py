...
# 공정 카테고리 라우팅 키워드
# - 한글 키워드: 부분 문자열 매칭 (한글은 단어 경계 정규식이 잘 동작하지 않음)
# - 영문 키워드: \b 단어 경계로 매칭해 "etc", "cd", "mask" 같은 짧은 토큰의 오탐 방지
KOREAN_KEYWORDS = {
    "photo": ["노광", "감광액"],
    "etch": ["식각"],
    "deposition": ["증착", "박막"],
...
def rewrite_query_with_glossary(query: str) -> str:
    ...
    for abbr, full_term in TERM_GLOSSARY.items():
        # 단어 경계 매칭으로 오탐 완화 (예: "AF" 가 다른 단어 일부로 매칭되는 것 방지)
...
def route_question(query: str) -> str:
    ...
    for category, words in KOREAN_KEYWORDS.items():
        if any(w in query for w in words):  # 원문(query) 기준, 한글은 대소문자 없음
            return category
...
def _compute_manifest(manual_dir: str, pdf_files: list) -> str:
    """manuals 폴더 내 PDF 목록 + 수정시각 기반 해시. 파일이 추가/변경되면 값이 달라짐."""
    parts = []
...
def create_multi_pdf_rag_chain(manual_dir="manuals", force_rebuild=False):
    ...
    # 매뉴얼 파일 목록/수정시각이 이전과 달라졌으면 자동으로 재빌드
    # (기존: 재색인 버튼을 누르지 않으면 FAISS는 옛 데이터, BM25/카테고리 필터는 최신 파일 목록을
    #  참조해 서로 어긋나는 문제가 있었음)
    ...
    # 카테고리별 BM25 리트리버를 여기서 한 번만 생성해 캐싱.
    # (기존: 질문마다 get_relevant_context 안에서 BM25Retriever.from_documents를 새로 만들어
    #  매뉴얼이 많아질수록 질문당 응답 지연이 계속 늘어나는 문제가 있었음)
